...
export class AuthService {
  ...
  constructor(private http: HttpClient, private processHTTPMsgService: ProcessHttpserviceService, private router: Router) { }
  ...
  sendLastname(lname: string) {
   ...
   logIn(user: any, seeker: boolean): Observable<any> {
     if (seeker) {
        return this.http.post<AuthResponse>(baseURL + 'users/seeker/login',
      ...
      .pipe( map(res => {
       ...
       catchError(error => this.processHTTPMsgService.handleError(error)));
        ...
        return this.http.post<AuthResponse>(baseURL + 'users/provider/login',
      {'username': user.username, 'password': user.password})
      .pipe( map(res => {
        const helper = new JwtHelperService();
        console.log(helper.decodeToken(res.token)._id);
 ...
 getLastName(): Observable<string> {
  ...
  postCandidate(body: any): Observable<any> {
    return this.http.post<any>(baseURL + 'users/seeker', body);
  }

  postCandidateUpdate(id: string, body: string): Observable<any> {
    return this.http.put<any>(baseURL + 'users/' + id + '/update/seeker', body);
  }

  postRecruiter(body: any): Observable<any> {
    return this.http.post<any>(baseURL + 'users/recruiter', body);
  }

  postRecruiterUpdate(id: string, body: any): Observable<any> {
    return this.http.put<any>(baseURL + 'users/' + id + '/update/recruiter', body);
  }
}
